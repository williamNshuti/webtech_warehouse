...
public class UserServiceImpl implements UserService {
    ...
    public boolean deleteUser(Long id) {
        ...
        if (byId.isPresent()) {
            ...
            for (AppClient client : all) {
                for (Warehouse warehouse : byId.get().getWarehouse_offers()) {
                    client.getWarehouse_matches().remove(warehouse);
                    client.getSaved_theWarehouses().remove(warehouse);
                }
    ...
    @Override
    public void findAndRemoveWarehouseFromClientsRecords(Warehouse warehouse) {
        List<AppClient> all = this.appClientRepository.findAll();

        for (AppClient appClient : all) {
            appClient.getSaved_theWarehouses().remove(warehouse);
            appClient.getWarehouse_matches().remove(warehouse);
        }
    }
