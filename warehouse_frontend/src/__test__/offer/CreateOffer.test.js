...
it("input should be initially empty", () => {
  const name = screen.getByLabelText("TheWarehouse Name");
...
it("change value of input element works correctly", () => {
  ...
  userEvent.type(name, "warehouse");
  expect(name.value).toBe("warehouse");
...
it("should show error message on invalid input", () => {
  ...
  const errorName = screen.getByText(
    /Warehouse name must be at least 3 characters long/i
  );
...
it("should create offer successfully", async () => {
  ...
  expect(mockAxios.post).toHaveBeenCalledWith(`/theWarehouses`, info);
