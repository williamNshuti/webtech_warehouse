...
import CreateOfferDataService from "../../../../../api/warehouse/CreateOfferDataService";
...
const CreateOffer = () => {
  ...
  const validate = () => {
    ...
    if (!info.name) {
      ...
      errors.name = "Warehouse name must be at least 3 characters long";
    }

    if (!info.slogan) {
      errors.slogan = "Slogan is required";
    } else if (info.slogan.length < 2 || info.slogan.length > 90) {
  ...
  const submitHandler = (event) => {
    ...
    if (Object.keys(errors).length === 0) {
      ...
      const uploaders = filesToUpload.map(async (file) => {
        ...
        formData.append("tags", `dsbs0e5f4, theWarehouse`);
        formData.append("upload_preset", "sc6qm4ea");
        formData.append("timestamp", (Date.now() / 1000) | 0);

        return await axios
          .post(
            "https://api.cloudinary.com/v1_1/dsbs0e5f4/image/upload",
            formData,
            {
    ...
    <>
      <main className={styles.offer_main}>
        ...
        <form className={styles.form_offer} onSubmit={submitHandler}>
          <OfferInputField
            type="text"
            placeholder="ex: Office, Electronics..."
            label="Offer Name"
            name="name"
            error={errors.name}
